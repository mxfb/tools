import { clampNumber, randomIntRange, shuffleArray } from "../common";
...
export function packAreasAndTiles(initialShape: Layout, dimensions: Dimensions, tileCoverage: number, tileDensity: { A: TileDensity, B: TileDensity }, tileEasing: TileEasing,  tileFormat?: string): Tiles {
    ...
    const filteredAreas = shuffleArray(areas.filter((area) => area.w !== 0 && area.h !== 0)); // We're shuffling to make sur we get different areas covered if necessary
