...
export type InnerGravity = 'top-left'
  | 'top-center'
  | 'top-right'
  | 'left-center'
  | 'center'
...
export async function innerResize (
  imageSharp: sharp.Sharp,
  params: InnerResizeParams
): Promise<sharp.Sharp> {
  ...
  const outputDimensions = params.outputDimensions !== undefined
  ...
  const innerContainedDimensions = Thumbnails.Layout.getContainedDimensions(
  ...
  const innerPositions = getInnerPositions(
    params.innerGravity ?? 'center', {
      w: innerDimensions.width,
      h: innerDimensions.height
    }, innerContainedDimensions, {
  ...
  return composedSharp
}

function getInnerPositions (
  innerGravity: InnerGravity,
  innerDimensions: { w: number, h: number },
  innerContainedDimensions: { w: number, h: number },
  outerDimension: { w: number, h: number }
) {
  switch (innerGravity) {
    case 'top-left':
    ...
    case 'bottom-right':
    return Thumbnails.Layout.alignBottomRight(innerDimensions, innerContainedDimensions, outerDimension);
    default:
    return { x: 0, y: 0 }
  }
}
